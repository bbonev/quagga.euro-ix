...
bgp_dump_open_file (bgp_dump_control bdc, struct vty* vty)
{
  ...
  tmp  = XCALLOC(MTYPE_TMP, PATH_MAX * 2) ;
  path = tmp + PATH_MAX ;
  ...
  ret = strftime(path, PATH_MAX, bdc->template, &tm) ;

  if (ret != 0)
    {
      mode_t oldumask ;

      if (path[0] != DIRECTORY_SEP)
        {
          snprintf (tmp, PATH_MAX, "%s/%s", vty_get_cwd(), path) ;
          path = tmp ;
        } ;

      oldumask = umask(0777 & ~LOGFILE_MASK);
      fd = open(path, O_WRONLY | O_CREAT, LOGFILE_MASK) ;
      ...
      if (fd < 0)
        {
          ...
          zlog_warn("Failed to open %s dump file %s: %s", typename, path,
                                                           errtoa(err, 0).str) ;
          if (vty != NULL)
  ...
  if (fd >= 0)
    {
      ...
      bd->filename = XSTRDUP(MTYPE_BGP_DUMP, path) ;
      bd->fd       = fd ;
